...
def gen_adatas(root, model):
    adata = sc.read(os.path.join(root, 'breastCancer2.h5ad'))
    adata.var_names_make_unique()
    sc.pp.normalize_total(adata, target_sum=1e4)
    sc.pp.log1p(adata)

    img = cv2.imread(os.path.join(root, 'breastCancer2.tif'))
    img = cv2.flip(img, 0)
    ...
    drop = []
    for i, coor in enumerate(adata.obsm['spatial']):
        x, y = coor
        ...
        if not img_p.numel():
            ...
            if y > height:
                y = height - 60  
            img_p = img[:, int(y-60):int(y), int(x-60): int(x)]
...
def infer(opt, r=0):
    ...
    for i in pth_list:
        ...
        colors = {'desmoplastic changes': '#1f77b4',
                'lymphocytes': '#ff7f0e',
                # 'necrosis and hemorrhage': "#2ca02c",
                'tumors': "#2ca02c"
            }
        cs = ['' for i in range(pred.shape[0])]
        gt_cs = ['' for i in range(pred.shape[0])]
