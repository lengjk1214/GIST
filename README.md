...
model.load_state_dict(torch.load(local_dir, map_location="cpu"), strict=True)
model.eval()
```
...
def gen_adatas(root, id, img_name, model):
    ...
    for (x, y) in zip(adata.obs['cx'], adata.obs['cy']):
        ...
        # virchow2
        # model = model.to("cuda")
        # image_resized = image_resized.to("cuda")
        # with torch.inference_mode():
        #     output = model(image_resized.to(torch.float32))
        # class_token = output[:, 0]
        # patch_tokens = output[:, 5:]
        # embedding = torch.cat([class_token, patch_tokens.mean(1)], dim=-1)
        # embedding_cpu = embedding.detach().cpu()
        # feature  = embedding_cpu.numpy().flatten()  
        # patchs.append(feature)
